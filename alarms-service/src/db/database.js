...
const db = new sqlite3.Database(dbPath, (err) => {
    if (err) {
        ...
        console.log(`Connected to the SQLite database at ${dbPath} (alarms-service).`);
        initializeDb();
    }
});
...
function initializeDb() {
    ...
    try {
        ...
        db.exec(schema, (err) => {
            if (err) {
                console.error("Error initializing alarms database schema:", err.message);
            } else {
                console.log("Alarms database schema initialized or already exists.");
            }
        });
    } catch (readErr) {
        console.error("Could not read alarms schema.sql:", readErr.message);
        process.exit(1);
    }
}
